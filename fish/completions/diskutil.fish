...
function __fish_diskutil_using_not_subcommand
    ...
    and not __fish_seen_subcommand_from image
...
# coreStorage
complete -f -c diskutil -n __fish_use_subcommand -a image -d "Manipulate disk images (attach, create, etc)"
complete -f -c diskutil -n '__fish_seen_subcommand_from image' -a attach -d "Attach a disk image as a device. Will return information about the attached image upon success."
complete -f -c diskutil -n '__fish_seen_subcommand_from image' -a info -d "Print info regarding a disk image. Will print information about the given image upon success."
complete -f -c diskutil -n '__fish_seen_subcommand_from image' -a create -d "Create a disk image. Either blank or from an existing source (disk / disk image)."
complete -f -c diskutil -n '__fish_seen_subcommand_from image' -a resize -d "Resize a given image to a specified size if applicable."

# gencomp: reading from `diskimage  --help `
complete -c diskutil -n '__fish_seen_subcommand_from image' -s v -l verbose -d 'Verbose output'
complete -c diskutil -n '__fish_seen_subcommand_from image' -l stdinpassphrase -d 'Provide the passphrase for the disk image via stdin'
complete -c diskutil -n '__fish_seen_subcommand_from image' -l plist -d 'Produce output in a plist format'
complete -c diskutil -n '__fish_seen_subcommand_from image' -s h -l help -d 'Show help information.'
# gencomp: reading from `diskutil attach --help `
complete -c diskutil -n '__fish_seen_subcommand_from image; and __fish_seen_subcommand_from attach' -s r -l readOnly -d 'Force the resulting device to be read-only.'
complete -c diskutil -n '__fish_seen_subcommand_from image; and __fish_seen_subcommand_from attach' -s b -l nobrowse -d 'Render any volumes invisible in applications such as'
complete -c diskutil -n '__fish_seen_subcommand_from image; and __fish_seen_subcommand_from attach' -l mountPoint -d '<mountPoint>'
complete -c diskutil -n '__fish_seen_subcommand_from image; and __fish_seen_subcommand_from attach' -l mountOptions -d '<mountOptions>'
complete -c diskutil -n '__fish_seen_subcommand_from image; and __fish_seen_subcommand_from attach' -l noMount -d 'Same as --mountPolicy=noMount'
# gencomp: reading from `diskutil info --help `

complete -c diskutil -n '__fish_seen_subcommand_from info\|create\|resize\|attach' --force-files

complete -c diskutil -n '__fish_seen_subcommand_from image; and __fish_seen_subcommand_from info' -l extra -d 'Retrieve extra information, if available.'
# gencomp: reading from `diskutil create --help `
complete -c diskutil -n '__fish_seen_subcommand_from image; and __fish_seen_subcommand_from create' -s e -l encrypt -d 'Encrypt the disk image using AES-256.'
# gencomp: reading from `diskutil resize --help `
complete -c diskutil -n '__fish_seen_subcommand_from image; and __fish_seen_subcommand_from resize' -s s -l size -d "<size>       New size of the disk image or 'min' to resize to"
complete -c diskutil -n '__fish_seen_subcommand_from image; and __fish_seen_subcommand_from resize' -l image-only -d 'Will resize only the image without the content,'