...
complete -f -c bun -a run        -n __fish_use_subcommand  -d  "Execute a file with Bun"
complete -f -c bun -a lint       -n __fish_use_subcommand  -d  "Run a package.json script"
complete -f -c bun -a test       -n __fish_use_subcommand  -d  "Run unit tests with Bun"
complete -f -c bun -a x          -n __fish_use_subcommand  -d  "Execute a package binary (CLI), installing if needed (bunx)"
complete -f -c bun -a repl       -n __fish_use_subcommand  -d  "Start a REPL session with Bun"
complete -f -c bun -a exec       -n __fish_use_subcommand  -d  "Run a shell script directly with Bun"
complete -f -c bun -a install    -n __fish_use_subcommand  -d  "Install dependencies for a package.json (bun i)"
complete -f -c bun -a add        -n __fish_use_subcommand  -d  "Add a dependency to package.json (bun a)"
complete -f -c bun -a remove     -n __fish_use_subcommand  -d  "Remove a dependency from package.json (bun rm)"
complete -f -c bun -a update     -n __fish_use_subcommand  -d  "Update outdated dependencies"
complete -f -c bun -a outdated   -n __fish_use_subcommand  -d  "Display latest versions of outdated dependencies"
complete -f -c bun -a link       -n __fish_use_subcommand  -d  "Register or link a local npm package"
complete -f -c bun -a unlink     -n __fish_use_subcommand  -d  "Unregister a local npm package"
complete -f -c bun -a publish    -n __fish_use_subcommand  -d  "Publish a package to the npm registry"
complete -f -c bun -a patch      -n __fish_use_subcommand  -d  "Prepare a package for patching"
complete -f -c bun -a pm         -n __fish_use_subcommand  -d  "Additional package management utilities"
complete -f -c bun -a build      -n __fish_use_subcommand  -d  "Bundle TypeScript & JavaScript into a single file"
complete -f -c bun -a init       -n __fish_use_subcommand  -d  "Start an empty Bun project from a blank template"
complete -f -c bun -a create     -n __fish_use_subcommand  -d  "Create a new project from a template (bun c)"
complete -f -c bun -a upgrade    -n __fish_use_subcommand  -d  "Upgrade to latest version of Bun."