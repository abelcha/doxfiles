function ollama_model_list
    ...
    bkt --ttl 10m -- xh --follow https://ollama.ai/library 2>/dev/null | pup --indent 4 'a[href^=/library] text{}' | sd '\s{5,}' '\t' #| cut -f 4,5 | grep -E '^.{10,250}$' | grep -E -v 'dolphin-llama3|llama-pro|stablelm2|magicoder'
end
