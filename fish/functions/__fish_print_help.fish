function __fish_print_help --description 'Print help message for the specified fish function or builtin' --argument-names item --argument-names error_message
    switch $item
        case !
            set item not
        case .
    ...
    begin
        ...
        set -l lessopts isRF
        if type -q less; and test (less --xversion | string match -r 'less (\d+)')[2] -lt 530 2>/dev/null
