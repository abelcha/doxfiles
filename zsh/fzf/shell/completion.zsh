...
if ! declare -f _fzf_compgen_path > /dev/null; then
  _fzf_compgen_path() {
    echo "$1"
    command find -L "$1" \
      -name .git -prune -o -name .git -prune -o -name .hg -prune -o -name .svn -prune -o \( -type d -o -type f -o -type l \) \
...
if ! declare -f _fzf_compgen_dir > /dev/null; then
  _fzf_compgen_dir() {
    command find -L "$1" \
      -name .git -prune -o -name .git -prune -o -name .hg -prune -o -name .svn -prune -o -type d \
      -a -not -path "$1" -print 2> /dev/null | sed 's@^\./@@'
  }
fi
...
fzf-completion() {
  ...
  if [ ${#tokens} -gt 1 -a "$tail" = "$trigger" ]; then
    ...
    if eval "type _fzf_complete_${cmd} > /dev/null"; then
      ...
      zle reset-prompt
    # Check if command is a directory command
